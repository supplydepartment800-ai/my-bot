...
def analyze_coin():
    ...
    mode = request.args.get('mode', 'FUTURE').upper().strip() # SPOT හෝ FUTURE ලබා ගනී
    ...
    try:
        ...
        # Multi-Timeframe Technical Metrics [Kalin Thibba Logic]
        ...
        # 🚨 ANTI-LATE ENTRY & LATE DUMP DETECTION LOGIC [Kalin Thibba Logic]
        ...
        # Verified Trends for Safe Entries
            ...
            alert = f"STRONG CONFLUENCE: 1H, 4H & 1D trends are aligned upwards. Perfect for a safe {mode.lower()} entry."
            ...
            if mode == "FUTURE":
                ...
                # Spot position handles bearish market differently (Avoid buying)
                side = "WAIT / BEARISH TREND"
                status = "BEARISH"
                alert = "SPOT WARNING: Market trend is strongly bearish. Do not buy Spot assets right now. Wait for reversal."

        # Leverage adjustments based on selected mode
        leverage_display = "1x (Spot - No Leverage)" if mode == "SPOT" else "3x - 5x (Swing Recommended)"
